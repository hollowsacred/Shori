<template>
  <header class="header">
    ...
    :class="{'justify-end': isSuperAdmin}"
    >
      <nav class="header-nav-links" v-if="!isSuperAdmin">
        <router-link :to="{ name: 'category', params: { gender: 'formen' }, }">Мужчинам</router-link>
        <v-divider vertical />
        <router-link :to="{ name: 'category', params: { gender: 'forwomen' }, }">Женщинам</router-link>
        <v-divider vertical />
        <router-link :to="{ name: 'category', params: { gender: 'forkids' }, }">Детям</router-link>
      </nav>
      <router-link to="/" class="header__logo" v-if="!isSuperAdmin">
        <img
          height="60"
      ...
      <div class="header__links">
          ...
          <div @click="navigateToCart" class="header__cart" v-if="!isSuperAdmin">
...
export default {
  setup() {
    ...
    const logoutFromAccount = () => {
      ...
      store.commit("authStore/setIsSuperAdmin", false);
